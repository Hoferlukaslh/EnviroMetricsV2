...
Item {
    ...
    // "YYYY-MM-DDTHH:MM:SS" -> "ddd dd.MM" (ex: "mar. 07.07"), localisé
    function formatDayLabel(ts) {
        if (!ts || ts.length < 10) return "";
        var y = parseInt(ts.substring(0, 4));
        var mo = parseInt(ts.substring(5, 7)) - 1;
        var d = parseInt(ts.substring(8, 10));
        return Qt.formatDate(new Date(y, mo, d), "ddd dd.MM");
    }
    ...
    Canvas {
        ...
        onPaint: {
            ...
            // Un repère par changement de jour (minuit), avec le jour + la date,
            // plutôt que juste l'heure : plus utile sur une série de plusieurs jours.
            var dayTicks = [];
            for (var di = 0; di < data.length; di++) {
                var dts = data[di].timestemp;
                if (!dts) continue;
                if (di === 0 || dts.substring(11, 13) === "00") {
                    dayTicks.push(di);
                }
            }
            var lastLabelX = -Infinity;
            for (var dt = 0; dt < dayTicks.length; dt++) {
                var idx = dayTicks[dt];
                var tx = xAt(idx);
                // évite que deux labels se chevauchent
                if (tx - lastLabelX < 34 && idx !== data.length - 1) continue;
                lastLabelX = tx;

                ctx.strokeStyle = Qt.rgba(Kirigami.Theme.textColor.r, Kirigami.Theme.textColor.g, Kirigami.Theme.textColor.b, 0.3);
                ctx.beginPath();
                ctx.moveTo(tx, top + h);
                ctx.lineTo(tx, top + h + 3);
                ctx.stroke();

                ctx.textAlign = (idx === 0) ? "left" : (idx >= data.length - 2 ? "right" : "center");
                ctx.fillText(root.formatDayLabel(data[idx].timestemp), tx, top + h + 11);
            }
            ctx.textAlign = "left";
    ...
    Rectangle {
        ...
        PlasmaComponents.Label {
            ...
            text: {
                ...
                var ts = p.timestemp || "";
                var dayLabel = root.formatDayLabel(ts);
                var timeLabel = ts.length >= 16 ? ts.substring(11, 16) : "";
                return dayLabel + " " + timeLabel + " : " + p.value.toFixed(root.decimals) + root.unit;
            }
        }
    }
}
